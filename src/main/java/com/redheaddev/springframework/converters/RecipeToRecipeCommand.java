    ...
    public RecipeCommand convert(Recipe source) {
        ...
        if (source.getIngredients() != null && source.getIngredients().size() > 0){
            source.getIngredients()
                    .forEach(ingredient -> {
                        IngredientCommand ingredientToAdd = ingredientConverter.convert(ingredient);
                        ingredientToAdd.setRecipeId(source.getId());
                        command.getIngredients().add(ingredientToAdd);
                    });
        }
